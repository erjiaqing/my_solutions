...
kkIzWhQ5Rxd/vnM2QQr9Cxa2J9GXEV3kGDiZV90+PCDSVGY4VgF8y7GedI1h",
    ...
    let plaintext = aes_128_cbc_decrypt("YELLOW SUBMARINE".as_bytes(), &ciphertext, &vec![0; 16]);
    println!("{:?}", String::from_utf8_lossy(&plaintext));

    let plain_text = random_bytes(123);
    let random_key = random_bytes(16);
    let random_iv = random_bytes(16);

    let encrypt_decrypt_loop = aes_128_cbc_decrypt(
        &random_key,
        &aes_128_cbc_encrypt(&random_key, &plain_text, &random_iv),
        &random_iv,
    );

    assert_eq!(encrypt_decrypt_loop, plain_text);
}
